...
def crossbeam_dimple_evap(s, toENDBFIELD,extrawfm=[]):
	...
	for exwf in extrawfm:
		print "UUSING bfield extend"
		exwf.extend(bfield.dt)
		wfms.append(exwf)
...
class ipg_wave(wfm.wave):
	...
	def follow(self, odtpow):
		...
		odtpowFCPOW = physics.inv( 'odtpow', odtpowV)
		
		print "...Setting IPG to follow the evap ramp"
		
		ipg = odtpowFCPOW + 10.*ipgmargin/100. 
		
		ipg[ ipg>10.0 ] = 10.0
		ipg[ ipg<10.*ipgmin/100.] = 10.*ipgmin/100.
