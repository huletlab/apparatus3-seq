...
print "\n----- odt_uv_hf_evap_repump_recompress.py -----\n"
...
#Add Recompess Features
holddt = float(report['RECOMPRESS']['holddt'])
rampdt = float(report['RECOMPRESS']['rampdt'])
cpowf = float(report['RECOMPRESS']['cpowf'])
trapdt = float(report['RECOMPRESS']['trapdt'])
odtpow.appendhold(holddt)
odtpow.odt_linear(cpowend,cpowf,rampdt)
odtpow.appendhold(trapdt)
s.analogwfm_add(evap_ss,[odtpow])


# ENDEVAP should be equal to image
s.wait(image+holddt+rampdt)
